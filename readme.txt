=== Banorte Woocommerce ===
Contributors: Saul Morales Pacheco
Donate link: https://saulmoralespa.com/
Tags: commerce, banorte, banco, México, woocommerce, ecommerce, shop, tienda, store, cart, store, sales, sell
Requires at least: 4.9.5
Tested up to: 4.9.7
Requires PHP: 5.6.0
Stable tag: 1.0.0
License: GNU General Public License v3.0
License URI: http://www.gnu.org/licenses/gpl-3.0.html

Integration of banorte bank with woocommerce

== Description ==

Banorte Woocommerce is a payment method plugin for Woocommerce, integrates the banorte bank of Mexico allowing to receive payments through credit cards, Visa Mastercad

== Installation ==


1. Download the plugin
2. Enter the administrator of your WordPress.
3. Enter Plugins / Add-New / Upload-Plugin.
4. Find the plugin downloaded on your computer and upload it like any other file.
5. After installing the .zip you can see it in the list of installed plugins, you can activate or deactivate it.
6. To configure the plugin you must go to: WooCommerce / Adjustments / Finish Purchase and Locate the tab Banorte Woocommerce.
7. Make sure to fill in all the required fields of your configuration in relation to the access provided in your Banorte account


== Frequently Asked Questions ==

= Countries in which its use is available? =

Mexico

= How do I prove its operation? =

In the payment medium settings, you can choose the different states of the test environment

= What should I take into account for production? =

**Request the certification and release to production. According to the manual, you should contact Payworks support and request the review process**

== Screenshots ==

1. Half payment configuration corresponds to screenshot-1.png

== Changelog ==

= 1.0.0 =
* Initial stable release

== Additional Info ==
**Contribute** [repository on github](https://github.com/saulmoralespa/woo-banorte)

== Credits ==
* [Saul Morales Pacheco](http://saulmoralespa) [@saulmoralespa](http://twitter.com/saulmoralespa)